...
describe("declared locales", () => {
  ...
  test("two or more is multilingual, and the FIRST is the default a page is stamped with", () => {
    expect(caps({ locales: ["cs", "en"] })).toEqual({ locales: ["cs", "en"], defaultLocale: "cs", multilingual: true, pagesByType: true, siteFurniture: true, codeDefinedTypes: true, typeDeletion: true, mediaTerms: true, mediaDownload: true });
  });
  ...
  test("an empty declaration falls back rather than leaving a page with no locale", () => {
    expect(caps({ locales: [] })).toEqual({ locales: ["en"], defaultLocale: "en", multilingual: false, pagesByType: true, siteFurniture: true, codeDefinedTypes: true, typeDeletion: true, mediaTerms: true, mediaDownload: true });
  });
});
