...
describe("the preview href", () => {
  test("with no site route declared, it points at the CMS's own redeem endpoint", () => {
    ...
    expect(pagePreviewHref(MINT, { resolve })).toBe("https://cms.example.com/cms/preview?token=abc.def");
  });

  test("a declared site route gets the TOKEN, not the CMS path", () => {
    expect(pagePreviewHref(MINT, { siteUrl: "/preview", resolve })).toBe("/preview?token=abc.def");
  });

  test("an absolute site route works too — the site is often a different origin", () => {
    expect(pagePreviewHref(MINT, { siteUrl: "https://www.example.com/preview", resolve }))
  ...
  test("a route that already has query params keeps them", () => {
    ...
    expect(pagePreviewHref(MINT, { siteUrl: "/preview?layout=bare", resolve }))
      .toBe("/preview?layout=bare&token=abc.def");
  });
  ...
  test("the token is url-encoded", () => {
    ...
    const href = pagePreviewHref({ url: "/cms/preview", token: "a+b/c=d" }, { siteUrl: "/preview", resolve });
    expect(href).toBe("/preview?token=a%2Bb%2Fc%3Dd");
  });
});
